...
the key out of the JavaScript bundle. The `owner` column on `connects` is a
record of who did the work, not an enforced boundary. For a team tracker on a
private Vercel URL that's a reasonable trade. To close it properly, move reads and
